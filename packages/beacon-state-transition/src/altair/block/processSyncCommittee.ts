...
export function processSyncAggregate(
  ...
  if (verifySignatures) {
    ...
    const signatureSet = getSyncCommitteeSignatureSet(state, block, participantIndices.yes);
  ...
  balances.set(proposerIndex, balances.get(proposerIndex) + syncProposerReward * participantIndices.yes.length);

  // Positive rewards for participants
  for (const index of participantIndices.yes) {
    balances.set(index, balances.get(index) + syncParticipantReward);
  }

  // Negative rewards for non participants
  for (const index of participantIndices.no) {
    balances.set(index, balances.get(index) - syncParticipantReward);
  }
}
...
export function getSyncCommitteeSignatureSet(
  ...
  if (!participantIndices) {
    const committeeIndices = state.epochCtx.currentSyncCommitteeIndexed.validatorIndices;
    participantIndices = syncAggregate.syncCommitteeBits.intersectYesValues(committeeIndices);
  }
