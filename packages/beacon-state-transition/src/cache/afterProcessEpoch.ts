...
import {computeActivationExitEpoch, computeProposers, getChurnLimit} from "../util";
import {CachedBeaconStateAllForks} from "./stateCache";
import {computeEpochShuffling} from "../allForks";
 ...
 * new epoch.
 */
export function afterProcessEpoch(state: CachedBeaconStateAllForks, epochProcess: IEpochProcess): void {
  ...
  epochCtx.proposers = computeProposers(
    state,
    epochCtx.currentShuffling.epoch,
    epochCtx.currentShuffling.activeIndices,
    epochCtx.effectiveBalanceIncrements
  );
  ...
  // ```
  epochCtx.afterEpochTransition(state.slot);
}
