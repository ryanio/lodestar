...
import {computeCommitteeCount} from "../../src/allForks";
import {
 ...
 * This is generated from Medalla state 756416
 */
export function generatePerformanceStatePhase0(pubkeysArg?: Uint8Array[]): BeaconStatePhase0 {
  if (!phase0State) {
    const pubkeys = pubkeysArg || getPubkeys().pubkeys;
    ...
    const bitArrayAllSet = BitArray.fromBoolArray(Array.from({length: MAX_VALIDATORS_PER_COMMITTEE}, () => true));
    ...
    state.previousEpochAttestations = Array.from({length: numPrevAttestations}, (_, i) => {
      ...
      return {
        aggregationBits: bitArrayAllSet,
        data: {
